...
def project_vec(a,b):
    ...
    if np.array_equal(make_unit(a),a) != 1:
    ...
    a_col = a.reshape((3, 1))  # (1,3)
    a_row = a.reshape((1, 3))  # (3,1)
    b_col = b.reshape((3, 1))  # (1,3)

    norm_a = np.linalg.norm(a_col)
    y = norm_a*norm_a
    
    # Compute projection matrix
    proj_matrix = (a_col @ a_row) / (a_row @ a_col)  # (3x3) / scalar

    # Apply projection to b
    result = (proj_matrix @ b_col).flatten()  # shape becomes (3,)
    
    print(result)
    origin = np.array([0, 0, 0])
    axis = plot_vec(result,origin,"r")
    plot_vec(b,origin,"g",axis)
    plot_vec(a,origin,"y",axis)
...
def plot_robot(q1, q2, q3,axis = None, plot = True):
    ...
    if(plot):
        plot_vec(p1, p0, "r",ax)       # σύνδεσμος 1
        plot_vec(p2, p1, "g", ax)       # σύνδεσμος 2
        plot_vec(p3, p2, "b", ax)       # σύνδεσμος 3
...
# helper function for C4
# def helper_plot(pA, pB, tf, points, joint_values, order=3):
#     """
#     Υπολογίζει την τροχιά μεταξύ δύο σημείων στον χώρο εργασίας και εμφανίζει:
#     - 3D τροχιά με χρώμα ανάλογο του χρόνου
#     - Θέση, ταχύτητα και επιτάχυνση των αρθρώσεων q1, q2, q3
#     - Θέση, ταχύτητα και επιτάχυνση των συντεταγμένων x, y, z

#     Args:
#         pA (array-like): αρχική θέση στον χώρο εργασίας [x, y, z]
#         pB (array-like): τελική θέση στον χώρο εργασίας [x, y, z]
#         tf (float): συνολικός χρόνος κίνησης
#         points (np.ndarray): πίνακας Nx3 με σημεία του workspace
#         joint_values (np.ndarray): πίνακας Nx3 με αντίστοιχα [q1, q2, q3]
#         order (int): βαθμός πολυωνύμου (3 ή 5)
#     """
